...
const MainCard = ({ data }) => {
  ...
  return (
    <Link
    to={`/stockdetail/${data.id}`} // Use the unique `id` or `_id` for dynamic routing
    >
    ...
    <div className="relative g  h-full md:h-[70vh] w-full bg-white shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 cursor-pointer group md:max-w-sm lg:max-w-md">
      ...
      <div className="p-4 relative z-10">
        ...
        <h2 className="text-lg font-bold text-black">{data.title[language]}</h2>
        ...
        <div className="mt-2 text-sm text-gray-700">
          ...
          <p>
            <span className="font-bold text-sm text-black">Weight Capacity : </span>{" "}
            {data.weightCapacity}
          </p>
          <p>
            <span className="font-bold text-sm text-black">Mileage : </span>{" "}
            {data.mileage}
          </p>
        </div>
      </div>
